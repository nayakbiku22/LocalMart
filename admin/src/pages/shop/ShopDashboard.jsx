...
function ShopDashboard() {
  const { sToken, totalProducts, getTotalProducts, getOrders, orders, totalOrders } = useContext(ShopContext)
  ...
  return (
    <div className='flex flex-col w-full mt-24 ml-[220px]'>
      ...
      <p className='text-sm'>Welcome back! Here's what's happening with your shop.</p>
      <div className='flex flex-row w-full gap-4 mt-3'>
        ...
        <div className='h-32 border-none w-[27%] rounded-md bg-violet-50 shadow-md border border-gray-400 opacity-[] cursor-pointer flex flex-row items-center justify-center font-semibold gap-3'>
          < HiOutlineCurrencyRupee className='w-10 h-10 p-1 rounded-md bg-violet-200' />
          <div className='flex flex-col'>
            <p className='text-md'>Total Sales</p>
            <p className='text-xl'>Rs. 12,000</p>
          </div>
        </div>
      </div>

      <div className='w-[85%] h-auto border rounded-sm border-gray-400 mt-8 flex flex-col p-3 mb-6'>
        <div className='flex flex-row justify-between'>
          <p className='font-semibold text-md'>Recent Orders</p>
          <Link to={"/shop/view-orders"} className='text-sm font-semibold text-blue-600'>View all Orders </Link>
        </div>
